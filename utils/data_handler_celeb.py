...
class DataHandler:
    def __init__(self, config):
        ...
        self._init_data_splits(config, subject_for_training, sequence_for_training, subject_for_validation,
    ...
    def _indexing_face_verts(self, indices):
        ret = []
        for idx in indices:
            if idx < self.length_voca:
                ret.append(self.face_vert_mmap[idx])
            else:
                ret.append(self.celeb_face_vert_mmap[idx - self.length_voca])
        return np.stack(ret)

    def _slice_data_helper(self, indices):
        face_vertices = self._indexing_face_verts(indices)
        face_templates = []
        processed_audio = []
        subject_idx = []
        for idx in indices:
    ...
    def _load_data(self, config):
        self._load_data_voca(config)
        self._load_data_celeb(config)
        # process index maps
        ...
        # for k, v in self.templates_data.items():
        #     print("  {}: {}".format(k, v.shape))

        # for k, v in self.processed_audio.items():
        #     for kk, vv in v.items():
        #         print(" ", k, kk, vv['audio'].shape, vv['audio'].dtype, vv['sample_rate'])

        # max_idx = 0
        # for sub in self.data2array_verts:
        #     for seq in self.data2array_verts[sub]:
        #         for frm, idx in self.data2array_verts[sub][seq].items():
        #             max_idx = max(idx, max_idx)
        #             print(" ", sub, seq, frm, idx)
        # print(max_idx, self.face_vert_mmap.shape)

    def _load_data_voca(self, config):
        ...
        self.length_voca = len(self.face_vert_mmap)
    ...
    def _load_data_celeb(self, config):
        ...
        print("Loading raw audio")
        celeb_raw_audio = pickle.load(open(raw_audio_path, 'rb'), encoding='latin1')
        self.raw_audio.update(celeb_raw_audio)

        print("Process audio")
        if os.path.exists(processed_audio_path):
            celeb_processed_audio = pickle.load(open(processed_audio_path, 'rb'), encoding='latin1')
        else:
            celeb_processed_audio =  self._process_audio(celeb_raw_audio)
            if processed_audio_path != '':
                pickle.dump(celeb_processed_audio, open(processed_audio_path, 'wb'))
        self.processed_audio.update(celeb_processed_audio)

        print("Loading index maps")
        delta_idx = len(self.face_vert_mmap)
        data2array_verts = pickle.load(open(data2array_verts_path, 'rb'))
        for sub in data2array_verts:
            for seq in data2array_verts[sub]:
                for frm in data2array_verts[sub][seq]:
                    data2array_verts[sub][seq][frm] += delta_idx
        self.data2array_verts.update(data2array_verts)
    ...
    def _init_data_splits(self, config, subject_for_training, sequence_for_training, subject_for_validation,
        ...
        # print(self.training_sequences)
        # print(self.get_num_training_subjects())
        # quit(1)
    ...
    def _get_subject_sequences(self, subject_sequence_list):
        ...
        for subj, seq in subject_sequence_list:
            ...
            face_vertices.append(self._indexing_face_verts(frame_array_indices))
